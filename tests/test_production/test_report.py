...
ENDPOINT_URL = '/production/report'
...
async def test_filter_by_institution_id(
    ...
    await create_research_report()  # Relatório "ruído"
    await create_research_report(researcher_id=researcher['id'])
...
async def test_filter_by_collection_id(
    client, create_research_report, create_collection_entry
):
    # Arrange
    await create_research_report()
    report_in_collection = await create_research_report()

    collection = await create_collection_entry(
        entry_id=report_in_collection['id'], type='REPORT'
    )
...
async def test_filter_by_star(
    ...
    stared_article = await create_research_report()
