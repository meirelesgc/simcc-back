...
ENDPOINT_URL = '/production/software'
...
async def test_filter_by_institution_id(
    client, create_software, create_institution, create_researcher
):
    ...
    await create_software()  # Software "ruído"
    await create_software(researcher_id=researcher['id'])  # Software-alvo
...
async def test_filter_by_group_name(
    ...
    # Arrange
    group = await create_research_group(name='Grupo de Engenharia de Software')
    linked = await link_researcher_to_research_group(
        research_group_id=group['id']
    )
    ...
    expected_count = 1
    params = {'group': 'Grupo de Engenharia de Software'}
...
async def test_filter_by_collection_id(
    client, create_software, create_collection_entry
):
    # Arrange
    await create_software()
    software_in_collection = await create_software()

    collection = await create_collection_entry(
        entry_id=software_in_collection['id'], type='SOFTWARE'
    )
...
async def test_filter_by_star(
    ...
    stared_article = await create_software()
