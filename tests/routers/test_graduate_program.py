...
async def test_graduate_program_validation_messy_data(client, session):
    ...
    response = client.get('/graduate_program_profnit', params={'graduate_program_id': str(gp_id)})
...
async def test_graduate_program_article_production(client, session):
    ...
    response = client.get(f'/graduate_program/{gp.graduate_program_id}/article_production', params={'year': 2020})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]['a1'] == 1
...
async def test_list_graduate_program_researcher(client, session):
    ...
    response = client.get('/graduate_program_researcher', params={'graduate_program_id': str(gp.graduate_program_id)})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]['name'] == res.name
