...
# Cache para evitar consultas repetidas ao banco para a mesma instituição
...
def process_program_row(pg):
    """
    Processa uma linha.
    Retorna: (dados_processados, mensagem_erro)
    Se sucesso, mensagem_erro é None. Se falha, dados_processados é None.
    """

    # 1. Validação de Situação
    if pg.get('situacao') != 'EM FUNCIONAMENTO':
        return None, "Ignorado: Situação não é 'EM FUNCIONAMENTO'"

    # 2. Processamento de Cidade/Estado
    try:
    ...
    # 4. Processamento de Cursos (Parsing Seguro)
    try:
        cursos_raw = pg.get('cursos', '[]')
        # Garante que é string antes de avaliar
    ...
    try:
        ...
        # Não falha o processo inteiro por data inválida, apenas loga warning se necessário
        pg['start'] = None

    # 6. Tratamento de Telefone
    try:
        pg['phone'] = ''.join(
    ...
    try:
        ...
        pg['periodicity'] = '/'.join(
            set(item.get('Nome', '') for item in regimes if isinstance(item, dict))
        )
    except Exception:
        pg['periodicity'] = ''
...
def insert_or_update_program(pg):
    ...
    try:
        ...
        logger.error(f"Erro de SQL ao inserir programa {pg.get('codigo')}: {e}")
        raise e
...
def main():
    ...
    for index, row in programs.iterrows():
        ...
        if program:
            ...
            # Aqui agrupamos os erros pela mensagem retornada
    ...
    # Remove contagem de sucessos para mostrar apenas problemas/ignores
