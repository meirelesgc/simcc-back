...
def main(researcher_ids=None, lattes_ids=None):
    ...
    sql_steps = [
        ("clean_bibliographic_year", "UPDATE bibliographic_production SET year = NULL WHERE YEAR !~ '^\\d+$';"),
        ("fix_specific_qualis", "UPDATE bibliographic_production_article ba SET qualis = 'A4' WHERE ba.issn = '17412242';"),
        ("set_docente_flag", "UPDATE researcher SET docente = true WHERE id IN (SELECT researcher_id FROM graduate_program_researcher);"),
        ("update_jcr_eissn", """
        ...
        """),
        ("cast_year_integer", "UPDATE bibliographic_production SET YEAR_ = YEAR::INTEGER"),
        ("clean_title_quotes", "UPDATE bibliographic_production SET title = translate(title, '''', ' ')"),
        ...
        """),
        ("split_great_area", "UPDATE researcher_production SET great_area_ = STRING_TO_ARRAY(great_area, ';');"),
        ...
        """),
        ("clean_guidance_html_tags", "UPDATE guidance SET title = regexp_replace(title, '<a[^>]*>|</a>', '', 'gi');"),
        ("link_group_leaders", """
    ...
    items_found = len(sql_steps) + 1  # 12 static SQLs + 1 quadrennial calculation
    ...
    try:
        ...
        items_succeeded = succeeded_steps if 'succeeded_steps' in locals() else 0
        items_failed = items_found - items_succeeded
