...
def list_admin_researchers(session):
    SCRIPT_SQL = text("""
        ...
        FROM public.researcher LIMIT 10;
    """)
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                logger.error(
                    'cnpq_att_failed', lattes_id=lattes_id, error=str(e)
                )
                return datetime.min
...
def download_xml(session, lattes_id, researcher_id):
...
def main():
    ...
    try:
        ...
        for i, researcher in enumerate(researchers):
            ...
            error = download_xml(session, lattes_id, researcher_id)
            if error:
