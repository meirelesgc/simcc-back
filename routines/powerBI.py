...
def supervisor():
    ...
    four_years = today.replace(year=today.year + 4)

    pending_next_year = csv[csv['planned_date_conclusion'] > next_year]
    count_next_year = pending_next_year.groupby(
        'supervisor_researcher_id'
    ).size()

    pending_four_years = csv[csv['planned_date_conclusion'] > four_years]
    count_four_years = pending_four_years.groupby(
        'supervisor_researcher_id'
    ).size()

    merged = (
        pd.DataFrame({
            'pending_next_year': count_next_year,
            'pending_four_years': count_four_years,
        })
        .fillna(0)
        .astype(int)
        .reset_index()
