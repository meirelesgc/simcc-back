...
async def enrich_researchers(session, researchers: list):
    """
    Enriches a list of researchers with additional data from multiple sources.
    Uses dictionary mapping for efficient association without Pandas.
    Returns the enriched list (may contain new dict objects if inputs were immutable).
    """
    if not researchers:
        return researchers

    # Ensure researchers are mutable (convert RowMapping/Pydantic to dict if needed)
    # or handle them as dicts throughout
    ...
    for r in researchers:
        if hasattr(r, 'model_dump'):  # Pydantic
            ...
            # Create a dict from any object that has .get() but isn't a dict
            # or simply try to convert common immutable types
            processed_researchers.append(dict(r))
        else:
            processed_researchers.append(r)

    # Extract IDs for batch querying using the processed list
    researcher_ids = []
    lattes_ids = []
    ...
    ufmg_map = {row['id']: row for row in ufmg_data}
...
async def list_co_authorship(session, researcher_id):
    ...
    df['initials'] = df['name'].apply(
        lambda name: ''.join(
            w[0] for w in str(name).replace('.', '').split() if w
        )
        if pd.notnull(name)
        else ''
    )
